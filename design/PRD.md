...
*   **设计细节**：
    1.  **超宽列布局**：表格的列名包含 `term_id` (词条 ID)、`zh_cn` (中文源词)、`context` (上下文描述)、`modified_by` (修改人)、`modified_time` (修改时间)，以及超过 15 种目标语言字段（如 `en_us`、`es_es`、`de_de`、`fr_fr`、`it_it`、`ja_jp`、`ko_kr` 等）。
    2.  **固定列 (Freeze Columns)**：为了在横向滚动（Scroll）时保证可读性，最左侧的 `term_id` 和 `zh_cn` 两列必须固定在左侧（CSS sticky positioning），不可被横向滑动掩盖。
    3.  **下拉切换版本**：主界面顶部提供版本下拉列表，列出当前 Base 下所有以版本命名的子表（如 `3.2`、`3.3`）。切换后，下方表格数据实时重载。
    4.  **实时检索**：支持对 `term_id` 和 `zh_cn` 的快速模糊搜索；提供“仅显示未翻译/缺失译文词条”的快捷过滤开关。

### 4.2 功能 F-02：一键录入与 Dify 智能翻译 (Term Creation & Dify Translate)
*   **需求说明**：在新词条入库或现有词条翻译缺失时，调用 Dify 翻译工作流进行翻译，并支持回写表格。
*   **交互与数据流**：
    1.  **用户操作**：用户点击“新增词条”按钮，弹出浮层表单，输入 `term_id`、`zh_cn`（中文）和可选的 `context`（上下文说明，如“用在设置菜单的第一行，限制10个字符”）。
    2.  **触发翻译**：用户点击“调用 Dify 翻译”，前端妙搭插件向配置的 Dify 工作流发送 HTTP 请求，Payload 携带 `{ term_id, zh_cn, context, target_languages }`。
    3.  **大模型翻译响应**：Dify 返回标准的 JSON 格式，如 `{"en_us": "Average Speed", "es_es": "Velocidad media", ...}`。
...
*   **计算与渲染逻辑**：
    ...
    2.  **数据差分算法**：
        *   同时拉取 $V_{curr}$ 和 $V_{prev}$ 的全量数据（以 `term_id` 为唯一键）。
    ...
    3.  **修改字段详情**：点击被修改的行，可以展开抽屉查看具体字段的对比（如：`英文(en_us): Avg Speed ➔ Average Speed`）。
...
*   **业务校验规则**：
    ...
    3.  **动态扩充语种列**：
        *   妙搭插件读取 CSV 文件的表头。
        *   如果 CSV 表头中存在当前多维表格中不存在的语种编码列（例如新增了意大利语 `it_it` 或荷兰语 `nl_nl`），系统**会自动调用多维表格 API 在新创建的子表最右侧追加这些文本字段列**。
    4.  **分批次批量写入**：
...
| **CSV 必填列** | CSV 文件必须包含词条 ID 列和中文源词列 | 上传 CSV 解析时 | 若缺少 `term_id`（或 `唯一标识`）及 `zh_cn`（或 `中文`），提示：“解析失败，CSV必须包含 term_id 和 zh_cn 列” |
