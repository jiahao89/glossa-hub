...
## 🖥️ 运行模式对比 (插件模式 vs 离线模式)

根据运行环境的不同，GlossaHub 平台会自动激活相应的运行模式：

| 功能特性 | 🔌 插件模式 (飞书多维表格组件) | 🌐 离线模式 (浏览器独立访问) |
| :--- | :--- | :--- |
| **激活场景** | 在飞书多维表格（Bitable）内部以自定义小组件运行 | 独立在 Safari/Chrome 等浏览器网页中访问平台地址 |
| **数据来源** | 飞书多维表格在线实时数据 | 本地 SQLite 缓存数据 / 离线内存模拟数据 |
| **数据写入** | **双向写入**。写入或清理词条会即时反映在飞书表格中，并自动同步存入本地 SQLite。 | **单向本地修改**。只修改本地内存或 SQLite 缓存，不影响飞书线上表格。 |
| **自动同步监听** | **支持**。通过 Bitable SDK 的事件监听器后台防抖同步飞书端的所有手工修改。 | **不支持**。无法与飞书云端通信，右上角显示“离线模式”状态标识。 |
| **“立即同步”按钮** | **可用**。一键强刷将线上飞书数据拉取并全量覆盖写入本地 SQLite。 | **置灰禁用**。处于离线状态无法连接多维表格 API。 |
| **版本比对** | 可在飞书当前的各子表（版本）之间进行实时双向差异化比对。 | 基于本地 SQLite 缓存的历史表格或内置模拟版本进行比对。 |
| **修改日志标记** | 记录详细的协作修改操作，且自动匹配当前飞书版本名称。 | 日志操作会自动标注 `(离线)` 字样，以示区分。 |
